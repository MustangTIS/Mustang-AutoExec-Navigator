...
- **Noise Reduction / ノイズ除去**: 
  ...
  - システム項目（はい/Yes）を隠し、ユーザーが追加した項目だけに集中。
...
1.  **Run as Admin (Mandatory)**: Right-click `AutoRun.bat` and select **"Run as Administrator"**.  
    `AutoRun.bat` を右クリックして**「管理者として実行」**してください。（※管理者権限がない場合、警告が表示され停止します）
